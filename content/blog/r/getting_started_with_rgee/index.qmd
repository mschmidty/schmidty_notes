...
Python 3.12.1
```
I have python 3.12.1 installed, the latest verions as of the writing of this post. Then you can create a `venv` from whatever directory you wish (cd there first) and initialize it. 
...
You should have `(rgee_env)` in your terminal prompt if the environment was successfully created. Now we need to install the dependancies earthengine-api and Numpy. From the terminal again (not the R terminal): 
...
Here we initialize my google cload account and then make sure my credentials are good. 
```{r eval=FALSE}
